...
def get_args():
    ...
    parser.add_argument('-nk', '--no-pokestops',
                        help=('Disables PokeStops from the map (including ' +
                              'parsing them into local db).'),
                        action='store_true', default=False)
    parser.add_argument('-nku', '--no-pokestops-update',
                        help=('Disables PokeStops savinf in DB.'),
                        action='store_true', default=False)
    ...
    if args.only_server:
        ...
        if args.pgpool_url:
            ...
            if args.pgpool_initial_accounts is None:
                raise Exception('Can\'t load accounts fromm PGPool')
